package run.app.repository.base;
...
import run.app.annotation.SensitiveConceal;
...
public interface BaseCommentRepository<COMMENT extends BaseComment>
     ...
     */
    @Query(
        "select new run.app.model.projection.CommentCountProjection(count(comment.id), "
            + "comment.postId) "
     ...
     */
    @Query(
        "select new run.app.model.projection.CommentChildrenCountProjection(count(comment"
            + ".id), comment.parentId) "
            + "from BaseComment comment "
